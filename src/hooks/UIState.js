...
export const useUIState = () => {
  ...
  const completeMenu = useMemo(() => {
    ...
    return _menu?.reduce((acc, item) => {
      if (!state.activeMenu.value?.startsWith(item.path) || !item.path)
  ...
  return {
    ...
    addLoadingForeground: () => {
      state.loading.set(state.loading.value + 1);
    },
    removeLoadingForeground: () => {
      state.loading.set(state.loading.value - 1);
    },
    isVisibleChangeProfilePictureDialog:
      state.isVisibleChangeProfilePictureDialog.value,
    toogleChangeProfilePictureDialog: (value) => {
      state.isVisibleChangeProfilePictureDialog.set(value);
    },
    addNotification: (message, severity = null) => {
      if (!severity) toast(message, defaultNotificationOptions);
      else toast[severity](message, defaultNotificationOptions);
    },
  };
};
